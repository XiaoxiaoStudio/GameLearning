...
namespace Air2000.ARPG
{
    [CustomEditor(typeof(MotionMachine))]
    public class InspecMotionMachine : UnityEditor.Editor
    {
        public static MotionMachine Instance;
        public static int CurrentSelectedMotionIndex;
        ...
        public static Motion CurrentSelectedMotion;

        public static int CurrentSelectedPluginIndex;

        public List<string> AllPluginNames = new List<string>();
        public static MotionPlugin CurrentSelectedPlugin;
        public static RoleMotionType GonnaCreateMotionType;
        ...
        {
            if ((target as MotionMachine) != Instance || Instance == null)
            {
                CurrentSelectedMotionIndex = 0;
                CurrentSelectedPluginIndex = 0;
        ...
        public override void OnInspectorGUI()
        {
            try
            {
                ...
                {
                    OpenMotionHeader = EditorUtils.DrawHeader("Motion: " + CurrentSelectedMotion.Type.ToString() + "  ||  Clip:  " + CurrentSelectedMotion.ClipName, true);
                }
                else
                {
                    OpenMotionHeader = EditorUtils.DrawHeader("Motion", true);
                }
                if (OpenMotionHeader)
                {
                    ...
                    if (GUILayout.Button("Add", GUILayout.Width(60), GUILayout.Height(15)))
                    {
                        ...
                        {
                            CharacterSystemUtils.LogError("motion name is illegal,please try another one"); return;
                        }
                    ...
                    CurrentSelectedMotionIndex = EditorGUILayout.Popup(CurrentSelectedMotionIndex, AllMotionNames.ToArray());
                    if (CurrentSelectedMotionIndex < Instance.Motions.Count && CurrentSelectedMotionIndex >= 0)
                    {
                        CurrentSelectedMotion = Instance.Motions[CurrentSelectedMotionIndex];
                    }
                    else
                    {
                        CurrentSelectedMotion = null;
                ...
                GUILayout.Space(5.0f);
                if (EditorUtils.DrawHeader("Plugin", true))
                {
                    if (CurrentSelectedMotion == null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            AllPluginType = EditorUtils.GetAllSubClass(typeof(MotionPlugin), null);
                            if (AllPluginType != null && AllPluginType.Count > 0)
                            {
                                ...
                                for (int i = 0; i < AllPluginType.Count; i++)
                                {
                                    ...
                                    string typeName = EditorUtils.GetTypeNameWithoutNamespcae(type.FullName);
                        ...
                        if (GUILayout.Button("Add", GUILayout.Width(60), GUILayout.Height(15)))
                        {
                            ...
                            {
                                CharacterSystemUtils.LogError("plugin identify name is illegal,please try another one"); return;
                            }
