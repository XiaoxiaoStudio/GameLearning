...
namespace Air2000 
{
    ...
    public class MovePlugin : MotionPlugin
    {
            ...
            if (Type == MoveType.Update || Type == MoveType.BeatBack || Type == MoveType.FlashMove)
            {
                ...
                switch (Type)
                {
                    case MoveType.Update:
                        newPos = Machine.Character.DestinationPosition;
                        break;
        ...
        private void MoveToTarget()
        {
            ...
            {
                travelTime = Vector3.Distance(Machine.Character.transform.position, Machine.Character.DestinationPosition) / MoveSpeed;
            }
            else
            {
            ...
            iTween.MoveTo(Machine.Character.gameObject, iTween.Hash("easetype", EaseType, "time", travelTime, "position", Machine.Character.DestinationPosition, "oncomplete", "OnTweenEnd", "oncompleteparams", this));
        }
        private void FaceToTarget()
        {
            float distance = Vector3.Distance(Machine.Character.DestinationPosition, Machine.Character.transform.position);
            if (distance < 0.3f)
            {
                return;
            }
