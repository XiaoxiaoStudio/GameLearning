...
namespace Air2000
{
    ...
    public class MotionPlugin
    {
        ...
        #endregion
        public virtual void OnMotionBegin(Motion motion) { if (DelayHandler != null && DelayHandler.Active) DelayHandler.Cancel(); CurrentStatus = Status.UnAction; }
        ...
        public virtual void ParseXML(SecurityElement element, Motion motion)
        {
            Motion = motion;
            float.TryParse(element.Attribute("BeginTime"), out BeginTime);
            float.TryParse(element.Attribute("EndTime"), out EndTime);
            bool.TryParse(element.Attribute("PlayOnMotionEnd"), out PlayOnMotionEnd);
            bool.TryParse(element.Attribute("DontStopOnMotionEnd"), out DontStopOnMotionEnd);
        }
