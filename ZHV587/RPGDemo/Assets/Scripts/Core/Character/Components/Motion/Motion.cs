...
namespace Air2000
{
    ...
    public class Motion
    {
        ...
        public delegate void ForceStopDelegate(Motion motion);
        ...
        public event ForceStopDelegate PostForceStop;
        ...
        public Character Character
        {
            get
            {
                return Machine == null ? null : Machine.Character;
            }
        }
        ...
        public MotionPlugin GetPlugin(string pluginName, System.Type type)
        {
            ...
            {
                CharacterSystemUtils.LogError("GetAction error caused by null action name"); return null;
            }
            if (type == typeof(MovePlugin))
            {
                for (int i = 0; i < MovePlugins.Count; i++)
        ...
        public void ParseXML(SecurityElement element, MotionMachine machine)
        {
            Machine = machine;
            float.TryParse(element.Attribute("Speed"), out Speed);
            ClipName = element.Attribute("Clip");
            WrapMode = (WrapMode)CharacterSystemUtils.TryParseEnum<WrapMode>(element.Attribute("WrapMode"));
            HowToFinish = (FinishMode)CharacterSystemUtils.TryParseEnum<FinishMode>(element.Attribute("Life"));
            ...
            if (pluginElements != null && pluginElements.Count > 0)
            {
                for (int i = 0; i < pluginElements.Count; i++)
                {
                    ...
                    string className = pluginElement.Attribute("Type");
                    if (string.IsNullOrEmpty(className)) continue;
                    Type type = System.Type.GetType(className);
                    if (type == null) continue;
                    MotionPlugin plugin = type.Assembly.CreateInstance(className) as MotionPlugin;
                    if (plugin == null) continue;
                    plugin.ParseXML(pluginElement, this);
                    AddPlugin(plugin);
                }
            }
        }
