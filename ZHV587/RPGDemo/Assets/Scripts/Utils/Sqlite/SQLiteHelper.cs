...
namespace Air2000
{
    ...
    {
        private static SQLiteHelper m_Instance;//类实例
        ...
        {
            AssetManager.LoadAssetAsync<TextAsset>(DATABASE_FILE_PATH, DATABASE_NAME, OnLoadedDB);
        }
        private void OnLoadedDB(AssetManager.Request req)
        {
            if (req != null && req.Asset != null)
            {
                ...
                if (textasset)
                {
                    CreateDBConnect(textasset.bytes);
                }
            }
        }
