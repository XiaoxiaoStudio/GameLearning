...
namespace Air2000
{
    public class GTimer : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            m_EventBatch = 0;
            while ((GTimer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
            {
                ...
                if (m_EventIterator < 0)
                {
                    ...
                    m_EventIterator = GTimer.m_Active.Count - 1;
                    break;
                }

                // prevent index out of bounds
                if (m_EventIterator > GTimer.m_Active.Count - 1)
                    m_EventIterator = GTimer.m_Active.Count - 1;

                // execute all due events
                if (Time.time >= GTimer.m_Active[m_EventIterator].DueTime ||	// time is up
                    GTimer.m_Active[m_EventIterator].Id == 0)					// event has been canceled ('Execute' will kill it)
                    GTimer.m_Active[m_EventIterator].Execute();
                else
                {
                    // handle pausing
                    if (GTimer.m_Active[m_EventIterator].Paused)
                        GTimer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                    else
                        // log lifetime
                        GTimer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
                }
        ...
        private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
        {
            ...
            if (m_MainObject == null)
            {
                ...
                m_MainObject.AddComponent<GTimer>();
            ...
            GTimer.m_EventCount++;
            m_NewEvent.Id = GTimer.m_EventCount;
            ...
            GTimer.m_Active.Add(m_NewEvent);
        ...
        /// </summary>
        private static void Cancel(GTimer.Handle handle)
        {
        ...
        public static void CancelAll()
        {
            ...
            for (int t = GTimer.m_Active.Count - 1; t > -1; t--)
            {
                GTimer.m_Active[t].Id = 0;
            }
        ...
        public static void CancelAll(string methodName)
        {
            ...
            for (int t = GTimer.m_Active.Count - 1; t > -1; t--)
            {
                if (GTimer.m_Active[t].MethodName == methodName)
                    GTimer.m_Active[t].Id = 0;
            }
        ...
        private class Event
        {
            ...
            private void Recycle()
            {
                ...
                if (GTimer.m_Active.Remove(this))
                    m_Pool.Add(this);
            ...
            private void Destroy()
            {
                ...
                GTimer.m_Active.Remove(this);
                GTimer.m_Pool.Remove(this);
        ...
        public class Handle
        {
            ...
            private GTimer.Event m_Event = null;	// timer we're pointing at
            ...
            public int Id
            {
                ...
                set
                {
                    ...
                    for (int t = GTimer.m_Active.Count - 1; t > -1; t--)
                    {
                        if (GTimer.m_Active[t].Id == m_Id)
                        {
                            m_Event = GTimer.m_Active[t];
                            break;
                        }
                    }
                    if (m_Event == null)
